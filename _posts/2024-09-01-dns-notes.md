...
information on configuring the entire system.

### Configure Local DNS server (User/Client Node)

This is done at the **User/Client** node, which is the local machine initially asking for DNS lookup.
It is logical to imagine that the local machine needs to know where its corresponding **Recursive Name Server**
is to send the very first lookup request.

On Linux machines, it used to be manually configured in `/etc/resolv.conf`; but now it is usually done
automatically by `systemd-resolved`, which is the internal DNS stub resolver.
