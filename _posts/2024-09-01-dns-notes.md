...
stub resolver. You can check the IP of the local DNS server resolver by using commands like:

```bash
# Use `dig` or `nslookup` to resolve the domain name of `google.com`
# These commands will display the IP address of the DNS server that
...
This shows the DNS server running on SOHO router?!

```bash
# CLI of NetworkManager
...
However, we usually cannot configure them directly, we only configure them via your domain registrar.
